...
The active configuration is `configs/targets.json`. It points at local
ignored copies under `raw/may20/`, stores the output path, aperture settings,
...
for the target, comparison, and check stars in `configs/targets.json`.
Then run:
