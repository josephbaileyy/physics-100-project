...
from project_pipeline import centroid_peak_in_box, detect_sources, load_config, robust_sigma, source_table


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/targets.json"
CHART = ROOT / "analysis/project/aavso/X42421BZ.json"
OUT = ROOT / "analysis/project/photometry/AM_CVn_sequence_X42421BZ"
PER_OUT = ROOT / "analysis/project/periodograms/AM_CVn_sequence_X42421BZ"
PLOT_OUT = ROOT / "analysis/project/plots"
FLAT = "flat_4s_masked_clipmean"
SATURATION = 58000.0
APERTURE_R = 6.0
ANN_IN = 12.0
ANN_OUT = 20.0
GOOD_REFERENCE_DIR = ROOT / "analysis/project/qc/AM_CVn/good_reference"
...
def ensure_dirs() -> None:
    for path in [OUT, PER_OUT, PLOT_OUT, ROOT / "analysis/project/aavso", GOOD_REFERENCE_DIR]:
...
def good_reference_sources() -> pd.DataFrame:
    ...
    manifest = pd.read_csv(
        ROOT / "analysis/project/reduced/AM_CVn" / f"reduction_manifest_{FLAT}.csv"
    ).sort_values("jd")
    ...
    path = ROOT / "analysis/project/reduced/AM_CVn" / row.reduced_file
...
def frame_photometry(cfg: dict, ref: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    reduced_dir = ROOT / "analysis/project/reduced/AM_CVn"
...
def plot_reference_overlay(ref: pd.DataFrame) -> None:
    if GOOD_REFERENCE_INFO.exists():
        ...
        ref_path = ROOT / "analysis/project/reduced/AM_CVn" / info.reduced_file
    else:
...
def main() -> None:
    ...
    seq.merge(ref[["label", "ref_x", "ref_y", "used_for_photometry"]], on="label").to_csv(
        ROOT / "analysis/project/aavso/X42421BZ_sequence_pixel_table.csv", index=False
    )
