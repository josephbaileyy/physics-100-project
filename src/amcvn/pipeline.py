...
DEFAULT_CONFIG = Path("observations/targets.json")
ACTIVE_TARGETS = ["AM_CVn"]
...
def command_run_all(args) -> None:
    ...
    try:
        ...
        print("Fill manual x/y positions in observations/targets.json, then rerun photometry and periodogram.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    reduce_p.add_argument("--target", default="all", choices=["all", *ACTIVE_TARGETS])
    ...
    prop_p.add_argument("--target", default="all", choices=["all", *ACTIVE_TARGETS])
    prop_p.add_argument("--flat", help="Use a flat-specific reduction manifest")

    phot_p = sub.add_parser("photometry")
    phot_p.add_argument("--target", default="all", choices=["all", *ACTIVE_TARGETS])
    ...
    per_p.add_argument("--target", default="all", choices=["all", *ACTIVE_TARGETS])
